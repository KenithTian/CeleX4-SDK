...
#define accResolution (2 * gravity / std::pow(2.0,15))
#define resolution (250.0 / std::pow(2.0, 15) / 180 * CV_PI)
...
FPGADataReader::IMUDATAType FPGADataReader::isIMUData(unsigned char data[4])
{
	...
	else if ((data[0] & 0x80) == 0 && (data[1] & 0x80) == 0 && (data[2] & 0x80) == 0x80)
	{
		if (0x00 == (data[3] & 0xC0))
		{
			...
			s_IMUData.x_GYROS_OFST = getIMU_X(data) * resolution;
			//std::cout << "GYROS_DATA: x_GYROS_OFST = " << s_IMUData.x_GYROS_OFST;
			return FPGADataReader::GYROS_OFST_A_DATA;
		}
		else if (0x40 == (data[3] & 0xC0))
		{
			s_IMUData.y_GYROS_OFST = getIMU_Y(data) * resolution;
			s_IMUData.z_GYROS_OFST = getIMU_Z(data) * resolution;
			//s_IMUData.t_GYROS_OFST = getIMU_T(data);
			s_IMUData.t_GYROS_OFST = s_uiEventTCounter;
		...
		{
			s_IMUData.x_ACC_OFST = getIMU_X(data) * accResolution;
